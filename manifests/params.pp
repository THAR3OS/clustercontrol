...
#   default settings for module clustercontrol init class
#
class clustercontrol::params {
  ...
  case $facts['os']['osfamily'] {
    ...
    'Debian': {
      $lsbmajdistrelease = 0 + $facts['os']['release']['major']
      if ($facts['os']['name'] == 'Ubuntu' and $lsbmajdistrelease > 12) or ($facts['os']['name'] == 'Debian' and $lsbmajdistrelease > 7){
        $typevar = type($lsbmajdistrelease)
        notify{"<<<<<<<<<<<<<dababies>>>>>>>>>>>>>The value is: ${facts['os']['name']} and ${facts['ipaddress_lo']} and ${$facts['os']['release']['codename']} and ${lsbmajdistrelease} and data-type is: ${typevar})": }
        $wwwroot          = '/var/www/html'
